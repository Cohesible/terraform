...
func NewInstaller(targetDir *Dir, source getproviders.Source, locks *depsfile.Locks) *Installer {
	return &Installer{
		targetDir: targetDir,
		source:    source,
...
// other, and not both.
func (i *Installer) EnsureProviderVersions(ctx context.Context, reqs getproviders.Requirements, mode InstallMode) (*depsfile.Locks, error) {
	...
	for provider, versionConstraints := range reqs {
		...
		if !mode.forceQueryAllProviders() {
			...
			if lock := i.locks.Provider(provider); lock != nil {
...
NeedProvider:
	...
	for provider, version := range need {
		...
		_, err := i.InstallProvider(ctx, provider, version, targetPlatform)
		if err != nil {
			errs[provider] = err
			continue
		}
	...
	// for use.
	for providerAddr := range i.locks.AllProviders() {
		if _, ok := reqs[providerAddr]; !ok {
			i.locks.RemoveProvider(providerAddr)
		}
	}

	if len(errs) > 0 {
		return i.locks, InstallerError{
			ProviderErrors: errs,
		}
	}
	return i.locks, nil
