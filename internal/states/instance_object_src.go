...
func (os *ResourceInstanceObjectSrc) Decode(ty cty.Type) (*ResourceInstanceObject, error) {
	...
	if os.AttrsFlat != nil {
		...
		if os.AttrMarks != nil {
			val = val.MarkWithPaths(os.AttrMarks)
		}
		if os.Pointers != nil {
			var markers []cty.PathValueMarks
			for _, p := range os.Pointers {
				vm := cty.NewValueMarks(marks.NewPointerAnnotation(p.Value, ""))
				markers = append(markers, cty.PathValueMarks{Path: p.Path, Marks: vm})
			}

			val = val.MarkWithPaths(markers)
		}
		if err != nil {
			return nil, err
		}
